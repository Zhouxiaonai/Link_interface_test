...
#noliving.jpg
sign_bg_avatar= DataDeal.get_pic_base64(r"D:\tasks\SenseLink\测试数据\打卡记录数据\record01\noliving.jpg")
login_avatar = DataDeal.get_pic_base64(r"D:\tasks\SenseLink\测试数据\打卡记录数据\登录人脸\loginme.jpg")
...
class TestRecordUp(unittest.TestCase):
    ...
    def test_mobileexist(self):
        rsapubRts = Login.get_rsapub(self.serveradd,self.rsaurl)
        testheader = Login.get_token(self.serveradd,self.loginurl,rsapubRts,self.secret,self.passwd,self.account)
        print("test_mobile_exist!!!")
        params = {'mobile':'15507558325'}
        # header =
        testrts = requests.get(self.serveradd+"/sl/v2/staff/check/exist",headers = testheader['request_headers'], params = params,verify = False)
        print(testrts.content.decode('utf-8'))
        pass

    def test_face_lgoin(self):
        print("face_login")
        ...
        print(params)
        header = {}
        testLoginRts = requests.post(self.serveradd+faceloginUrl,headers = header,json = params,verify=False)
        print(testLoginRts.content.decode('utf-8'))
        print("login-ook")
        pass

    def test_getrecord(self):
        print("test_getrecord!!!")
        getrecordurl = "/sl/v2/staff/record"
        rsapubRts = Login.get_rsapub(self.serveradd,self.rsaurl)
        testheader = Login.face_login(self.serveradd,self.mobile,self.country_code,self.loginavater,self.company,self.secret,self.passwd,self.account)
        self.params['timestamp'] = testheader['timestamp']
        params = {'type':'',
                  'data_time_from':'2020-09-29 00:57:37',
                  'date_time_to':'2020-09-29 23:57:37',
                  'device_sn':'',
                  'page':0,
                  'size':1000
        }
        recordRts = requests.get(self.serveradd+getrecordurl,headers = testheader['request_headers'],params=params,verify=False)
        print(recordRts.content.decode("utf-8"))
        pass
    def tearDown(self):
        print("end")
        pass
